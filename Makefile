...
# 1) run make once to create and update the po/jsonlistmodeltest.pot
# 2) copy the template file and set the name to the language you want to
#    translate to:   cp po/jsonlistmodeltest.pot po/en.po
# 3) edit the po file
...
APP_ID=jsonlistmodeltest.liu-xiao-guo
...
po/jsonlistmodeltest.pot: $(QMLJS_FILES)
	mkdir -p $(CURDIR)/po && xgettext -o po/jsonlistmodeltest.pot --qt --c++ --add-comments=TRANSLATORS --keyword=tr --keyword=tr:1,2 $(QMLJS_FILES) --from-code=UTF-8
